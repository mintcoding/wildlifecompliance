...
class PhysicalArtifact(Artifact):
    ...
    def details_data(self):
        ...
        return self.details_form_data_records.all()

    @property
    def storage_data(self):
        """ returns a queryset of form data records attached to Inspection (shortcut to InspectionFormDataRecord related_name). """
        return self.storage_form_data_records.all()
...
class PhysicalArtifactDetailsFormDataRecord(models.Model):
    ...
    physical_artifact = models.ForeignKey(PhysicalArtifact, related_name='details_form_data_records')
    ...
    def __str__(self):
        return "Physical Artifact {id} Details record {field}: {value}".format(
    ...
    def process_form(PhysicalArtifact, form_data, action=ACTION_TYPE_ASSIGN_VALUE):
        ...
        if action == PhysicalArtifactDetailsFormDataRecord.ACTION_TYPE_ASSIGN_COMMENT and\
        ...
        for field_name, field_data in form_data.items():
            ...
            if PhysicalArtifactDetailsFormDataRecord.INSTANCE_ID_SEPARATOR in field_name:
                [parsed_schema_name, instance_name] = field_name.split(
                    PhysicalArtifactDetailsFormDataRecord.INSTANCE_ID_SEPARATOR
                )
                schema_name = schema_name if schema_name else parsed_schema_name

            form_data_record, created = PhysicalArtifactDetailsFormDataRecord.objects.get_or_create(
            ...
            if action == PhysicalArtifactDetailsFormDataRecord.ACTION_TYPE_ASSIGN_VALUE:
                form_data_record.value = value
            elif action == PhysicalArtifactDetailsFormDataRecord.ACTION_TYPE_ASSIGN_COMMENT:
...
@python_2_unicode_compatible
class PhysicalArtifactStorageFormDataRecord(models.Model):
    ...
    physical_artifact = models.ForeignKey(PhysicalArtifact, related_name='storage_form_data_records')
    field_name = models.CharField(max_length=512, blank=True, null=True)
    schema_name = models.CharField(max_length=256, blank=True, null=True)
    instance_name = models.CharField(max_length=256, blank=True, null=True)
    component_type = models.CharField(
    ...
    def __str__(self):
        return "Physical Artifact {id} Storage record {field}: {value}".format(
            id=self.physical_artifact_id,
            field=self.field_name,
            value=self.value[:8]
        )
    ...
    def process_form(PhysicalArtifact, form_data, action=ACTION_TYPE_ASSIGN_VALUE):
        ...
        if action == PhysicalArtifactStorageFormDataRecord.ACTION_TYPE_ASSIGN_COMMENT and\
                not can_edit_comments and not can_edit_deficiencies:
            raise Exception(
                'You are not authorised to perform this action!')
        ...
        for field_name, field_data in form_data.items():
            ...
            if PhysicalArtifactStorageFormDataRecord.INSTANCE_ID_SEPARATOR in field_name:
                [parsed_schema_name, instance_name] = field_name.split(
                    PhysicalArtifactStorageFormDataRecord.INSTANCE_ID_SEPARATOR
                )
                schema_name = schema_name if schema_name else parsed_schema_name

            form_data_record, created = PhysicalArtifactStorageFormDataRecord.objects.get_or_create(
                physical_artifact_id=PhysicalArtifact.id,
                field_name=field_name
            )
            if created:
                ...
                form_data_record.instance_name = instance_name
                form_data_record.component_type = component_type
            if action == PhysicalArtifactStorageFormDataRecord.ACTION_TYPE_ASSIGN_VALUE:
                form_data_record.value = value
            elif action == PhysicalArtifactStorageFormDataRecord.ACTION_TYPE_ASSIGN_COMMENT:
                if can_edit_comments:
                    form_data_record.comment = comment
                if can_edit_deficiencies:
                    form_data_record.deficiency = deficiency
